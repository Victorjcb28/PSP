...
#Incluimos un Flowable, que en este caso es un p�rrafo.
    ...
    parrafo = Paragraph("Registro de Postulante ",cabecera)
...
#Damos un estilo BodyText al segundo p�rrafo, que ser� el texto a escribir.
    fila1=["Nombre","Apellido","Cedula","Sexo","Direccion"]
...
#Y lo incluimos en el story.
#Fila1
    cur.execute("Select postulante.nombre,postulante.apellido,postulante.cedula,postulante.direccion,registro.variable from Postulante,Registro order by oid DESC LIMIT 1 ")
    rs=cur.fetchone()
    ...
    fila1=["Diametro In","Area Flu","Velocidad","N° Reynold","Tipo Flujo"]
    tabla1=Table([fila1],colWidths=120)
    tabla1.setStyle([('BACKGROUND',(0,0),(-1,-1),colors.gray)])
    tabla1.setStyle([('INNERGRID',(0,0),(-1,-1),0.25,colors.black)])
    tabla1.setStyle([('BOX',(0,0),(-1,-1),0.25,colors.black)])
...
#Fila2
    ...
    fila4=[str(rs[6]),str(rs[7]),str(rs[8]),str(rs[9]),str(rs[10])]
...
#Fila3
    fila1=["Longitud Tuberia","Material","Rugosidad","Factor Friccion",""]
    tabla1=Table([fila1],colWidths=120)
    tabla1.setStyle([('BACKGROUND',(0,0),(-1,-1),colors.gray)])
    tabla1.setStyle([('INNERGRID',(0,0),(-1,-1),0.25,colors.black)])
    tabla1.setStyle([('BOX',(0,0),(-1,-1),0.25,colors.black)])
    ...
    fila4=[str(rs[11]),str(rs[12]),str(rs[13]),str(rs[0]),str()]
...
#Creamos un DocTemplate en una hoja DIN A4, en la que se muestra el texto enmarcado (showBoundary=1) por un recuadro.
    ...
    doc=SimpleDocTemplate("ReporteSimular.pdf",pagesize=landscape(letter),showBoundary=1)
